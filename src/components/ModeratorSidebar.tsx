/**
 * ModeratorSidebar — matches AdminSidebar structure exactly
 * Collapsed: w-14 (icons only) | Expanded: w-60
 * Active color: #C8DDD0 bg with #1A3A2A text (soft mint)
 */
...
// Moderator active tokens — 85% of admin (#1A3A2A)
const A = { bg: "#3C584A", text: "#FFFFFF", icon: "#FFFFFF" };
...
const ModeratorSidebar = ({
  ...
  const NavRow = ({
    icon: Icon, label, path, onClick, active = false, danger = false,
  }: {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden px-2 py-1" style={{ scrollbarWidth: "none" } as React.CSSProperties}>
        {moderatorSidebarConfig.sections.map((section, si) => (
          <div key={section.title} className={cn(si > 0 && "mt-3")}>
            ...
            <div className="space-y-0.5">
              {section.items.map((item) => (
                <NavRow key={item.path} icon={item.icon} label={item.label} path={item.path} active={isActive(item.path)} />
              ))}
            </div>
          </div>
        ))}
