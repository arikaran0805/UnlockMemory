import { ShieldCheck, Sparkles, Settings2, Zap, Tag, X, Package, PartyPopper, Copy, Ticket, Lock, Check } from "lucide-react";
    ...
    <div className="rounded-2xl border border-border bg-card p-6 space-y-4 shadow-md">
      ...
      {!selectedCareer ? (
        ...
        <>
          ...
          {/* FREE mode: Show upsell instead of bundle/promo */}
          {isFreeMode ? (
            <>
              <Separator />
              <div className="rounded-xl bg-gradient-to-br from-primary/5 to-accent/5 border border-primary/15 p-4 space-y-3">
                <p className="text-sm font-semibold text-foreground">Upgrade to Pro to unlock:</p>
                <ul className="space-y-1.5">
                  {["Course customization", "Bundle discounts", "Interview preparation"].map((item) => (
                ...
                <Button onClick={activateProMode} size="sm" className="w-full gap-1.5">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-primary">
              {formatPrice(isFreeMode ? breakdown.courseSubtotal : breakdown.finalTotal)}
            </span>
          </div>

          {/* 8. Savings Message (PRO only) */}
          {!isFreeMode && hasSavings && (
