...
export function EliminateWrongDescriptionPanel({
  ...
  const { likes, dislikes, userReaction, react, isAuthenticated: reactionsAuth } = useProblemReactions(problem.id, "eliminate");
  const { isBookmarked, toggleBookmark, isAuthenticated: bookmarksAuth } = useProblemBookmarks("eliminate");
