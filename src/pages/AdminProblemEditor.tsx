          ...
          <div className="sticky bottom-0 bg-background border-t pt-4 pb-4 z-40">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {/* Status toggle — no portal to avoid sticky footer z-index issues */}
                <FormField
                  control={form.control}
                  name="status"
                  render={({ field }) => (
                    <div className="flex items-center rounded-md border border-input overflow-hidden text-sm font-medium">
                      <button
                        ...
                        className={`px-3 h-9 border-r border-input transition-colors ${field.value === "draft" ? "bg-primary text-primary-foreground" : "bg-background text-muted-foreground hover:bg-muted"}`}
                      >
                        Draft
                      </button>
                      <button
                        type="button"
                        onClick={() => canPublish && field.onChange("published")}
                        disabled={!canPublish}
                        className={`px-3 h-9 transition-colors ${field.value === "published" ? "bg-primary text-primary-foreground" : canPublish ? "bg-background text-muted-foreground hover:bg-muted" : "bg-muted text-muted-foreground opacity-50 cursor-not-allowed"}`}
                      >
                        Published
                      </button>
                    </div>
